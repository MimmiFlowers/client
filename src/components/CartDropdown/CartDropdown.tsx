...
export const CartDropdown: React.FC<Props> = ({ onClose }) => {
    ...
    return (
        ...
        >
            <h3 className="text-xs font-medium uppercase tracking-[0.15em] text-[var(--color-muted)]">
                {t("cart.your_cart")}
            </h3>
                        ...
                        >
                            ...
                            <div className="flex-1">
                                <p
                                    ...
                                    role="button"
                                    tabIndex={0}
                                    className="cursor-pointer text-sm font-medium leading-tight text-[var(--color-obsidian)] transition-opacity duration-300 hover:opacity-70"
                                >
                                    {item.name}
                                </p>
                                ...
                                <div className="mt-1.5 flex items-center gap-1">
                                    <button
                                        ...
                                        className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-full border border-[var(--color-line)] text-xs text-[var(--color-muted)] transition-colors duration-300 hover:border-gray-400 hover:text-[var(--color-obsidian)]"
                                    >
                                        &minus;
                                    </button>
                                    <span className="w-6 text-center text-xs font-medium text-[var(--color-fg-soft)]">
                                        {item.quantity}
                                    </span>
                                    <button
                            ...
                            <div className="flex flex-col items-end gap-1">
                                ...
                                <button
                                    ...
                                    className="cursor-pointer text-xs font-light text-[var(--color-muted)] transition-colors duration-300 hover:text-red-400"
                                >
                                    {t("buttons.cancel")}
                                </button>
                            </div>
            ...
            {items.length > 0 && (
                <>
                    <div className="mt-3 space-y-1.5 border-t border-[var(--color-line)]/60 pt-3">
                        ...
                        <div className="flex justify-between border-t border-[var(--color-line)]/60 pt-2">
                            <span className="text-sm font-medium uppercase tracking-wider text-[var(--color-muted)]">
                                {t("cart.total")}
                            </span>
                            <span className="text-sm font-semibold text-[var(--color-obsidian)]">
                                {grandTotal.toLocaleString()} kr
                            </span>
                        </div>
                    </div>
                    <button
                        className="mt-4 w-full cursor-pointer rounded-full bg-[var(--color-obsidian)] py-2.5 text-xs font-medium uppercase tracking-wider text-white transition-opacity duration-300 hover:opacity-80"
                        onClick={() => {
                            onClose();
                            navigate("/Checkout");
                        }}
