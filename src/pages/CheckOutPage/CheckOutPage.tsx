            ...
            <input
                ...
                className={`w-full rounded-lg border bg-white px-4 py-2.5 text-sm text-gray-900 transition-all duration-200 outline-none placeholder:text-gray-300 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-400 ${
                    error
                        ...
                        : "border-gray-200 focus:border-[var(--color-leaf)] focus:ring-2 focus:ring-[var(--color-leaf)]/30"
                }`}
                placeholder={label}
            />
            {error && (
...
function SectionHeader({ step, title }: { step: number; title: string }) {
    return (
        <div className="mb-5 flex items-center gap-3">
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-leaf)]/40 text-xs font-semibold text-gray-700">
                {step}
            </span>
